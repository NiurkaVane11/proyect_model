...
const formatTimeForDB = (time) => {
  ...
  try {
    ...
    // Si es un timestamp completo, extraer solo la hora
...
// GET - Estadísticas generales (DEBE IR ANTES DE /:id)
router.get('/stats/general', async (req, res) => {
  try {
    ...
    const statsConvertidas = {
      total_distribuciones: Number(stats[0].total_distribuciones || 0),
      entregadas: Number(stats[0].entregadas || 0),
      pendientes: Number(stats[0].pendientes || 0),
      canceladas: Number(stats[0].canceladas || 0),
      total_bolsas_entregadas: Number(stats[0].total_bolsas_entregadas || 0),
      promedio_bolsas_por_entrega: Number(stats[0].promedio_bolsas_por_entrega || 0),
      panaderias_atendidas: Number(stats[0].panaderias_atendidas || 0)
    };
    
    res.json({
      success: true,
      data: statsConvertidas
    });
  } catch (error) {
...
router.get('/stats/panaderia/:id', async (req, res) => {
  try {
    ...
    const stats = await pool.query(`
      ...
      GROUP BY d.id_panaderia, p.nombre_comercial
    `, [id]);
...
// GET - Obtener una distribución por ID (DEBE IR AL FINAL DE LOS GET)
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!req.body.id_panaderia || !req.body.fecha_entrega || !req.body.cantidad_entregada) {
    ...
    const fecha_entrega = formatDateForDB(req.body.fecha_entrega);
    const hora_entrega = formatTimeForDB(req.body.hora_entrega);
    ...
    const values = [
      ...
      fecha_entrega,
      req.body.cantidad_entregada,
      req.body.nombre_receptor || null,
      req.body.cedula_receptor || null,
      req.body.firma_recibido || null,
      ...
      req.body.usuario_registro || null
    ];
...
router.put('/:id', async (req, res) => {
  try {
    ...
    // Formatear fecha y hora si vienen en el body
    const fecha_entrega = req.body.fecha_entrega ? formatDateForDB(req.body.fecha_entrega) : undefined;
    const hora_entrega = req.body.hora_entrega ? formatTimeForDB(req.body.hora_entrega) : undefined;

    // Construir el objeto de actualización
    const updates = { ...req.body };
    if (fecha_entrega !== undefined) updates.fecha_entrega = fecha_entrega;
    if (hora_entrega !== undefined) updates.hora_entrega = hora_entrega;
